...
- **목록을 가공하지 않는다**: `DataTable` 의 페이지네이션·정렬은 컨트롤과 인디케이터를 그리고
  의도(`onPageChange`, `onSortChange`)만 내보낸다. 실제 행 자르기·재배열은 소비 시스템이 끝낸 뒤
  `rows` 로 넘긴다.
...
| **순수 UI 상태 프로바이더** (승격된 예외, 아래 참조) | 이 라이브러리 | `src/lib/providers/**` |
| 데이터 패칭·상태·비즈니스 로직 | 소비 시스템(컨테이너) | 각 사내 애플리케이션 |
| 데모/프리뷰 (정적 예시 주입) | 이 저장소의 하네스 | `src/App.tsx`, `src/pages/**` |

### 예외 — UI 상태 프로바이더 (`src/lib/providers/**`)

`ToastProvider`(`useToast`)와 `AlertProvider`(`useAlert`)는 라이브러리에 포함된다.
토스트 큐·타이머, 다이얼로그 열림 상태·Promise 해소는 **화면 표현에 한정된 순수 UI
상태**라서(Do 항목의 "모달 열림/닫힘"과 같은 층위) 소비 시스템마다 재구현할 이유가 없기
때문이다. 단, 이 예외는 아래를 전제로 한다:

- Don't 목록은 그대로 적용된다 — HTTP·영속화·라우팅·비즈니스 로직·환경변수 분기 금지.
- 그리기 부품(`<Toast>`, `<AlertDialog>`, `<PromptDialog>`)은 여전히 독립 export 되므로,
  프로바이더를 쓰지 않고 소비 시스템이 자체 상태 관리로 직접 그릴 수도 있다.
- 라이브러리 컴포넌트가 이 프로바이더를 **쓰더라도 의존해서는 안 된다** — 프로바이더 없이도
  동작해야 하므로 `useToast`(없으면 throw) 대신 `useOptionalToast`(없으면 `null`)를 쓰고,
  토스트는 콜백을 안 준 경우의 **기본 안내**로만 쓴다(예: `FileUpload` 의 `onReject`).

인증(`AuthProvider`)처럼 데이터·영속화와 엮인 프로바이더는 예외가 아니며 하네스에 남는다.
