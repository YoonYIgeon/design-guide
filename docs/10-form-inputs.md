...
## Input — 입력 가능한 문자 제한 (`allow`)

"영소문자·숫자만", "숫자만" 처럼 **입력 자체를 특정 문자 집합으로 고정**하려면 `Input` 에
`allow` 정규식을 준다. 허용되지 않는 문자는 화면에 남지 않고 조용히 제거된다.

```tsx
// 한 글자씩 test 하는 "문자 클래스"다 — /^[a-z0-9]+$/ 처럼 값 전체를 매칭하지 않는다.
...
| prop | 타입 | 설명 |
| --- | --- | --- |
| `allow` | `RegExp` | 입력 가능한 **문자 집합**(한 글자 단위 `test`). 미지정이면 제한 없음 |

동작 규칙:

- **타이핑·붙여넣기·드래그&드롭·자동완성** 모두에 적용된다. 브라우저가 그린 값을 걸러
  되돌리는 방식이라 키 이벤트를 거치지 않는 입력도 동일하게 막힌다.
- `onChange` 에는 **이미 걸러진 값**이 담긴 이벤트가 전달된다. 컨테이너는 평소처럼
  `e.target.value` 를 그대로 저장하면 된다(react-hook-form `register` 도 동일).
- **IME 조합(한글 등) 중에는 거르지 않고**, 조합이 끝난 뒤 한 번에 거른다. 조합 중에 글자를
  건드리면 입력이 깨지기 때문이다. 그래서 한글은 "조합하는 동안 보였다가 확정 시 사라진다".
- 지운 글자 수만큼 **커서를 보정**하므로 문장 중간을 고쳐도 커서가 끝으로 튀지 않는다.
- 서로게이트 페어(이모지 등)는 코드 포인트 단위로 검사해 반쪽만 남지 않는다.
- `defaultValue`/초기 `value` 는 거르지 않는다 — **초기값 정규화는 컨테이너 책임**이다.
- `g`/`y` 플래그가 붙어 있어도 `lastIndex` 상태 없이 안전하게 동작한다.

> 프레젠테이션 전용 원칙([08](08-presentational-only.md))과의 관계: `allow` 는 "이 필드에
> 어떤 문자가 들어갈 수 있는가"라는 **입력 표면의 규칙**이지 도메인 검증이 아니다.
> `FileUpload` 의 `accept` 와 같은 성격이다. **형식·중복·범위 같은 최종 검증은 여전히
> 컨테이너와 서버의 몫**이며, 클라이언트 필터만 믿고 서버 검증을 생략하면 안 된다.
>
> 같은 규칙을 컴포넌트 밖(붙여넣기 전처리, 저장 직전 정규화 등)에서 쓰려면
> `filterAllowedChars(value, allow)` 를 라이브러리에서 그대로 가져다 쓴다.

`allow` 는 `Input` 을 감싸는 `AsyncInput` 에도 그대로 전달된다(예: 아이디 입력을
영문/숫자로 고정한 채 중복 확인). 대소문자 변환·자릿수 포맷팅(`010-1234-5678`) 처럼
**문자를 바꾸거나 끼워 넣는** 처리는 `allow` 의 몫이 아니다 — 컨테이너의 `onChange` 에서
값을 가공한다.

- 관련 네이티브 속성: `pattern` 은 **입력을 막지 않고** 제출 시 검증만 한다(`maxLength`,
  `inputMode` 와 함께 쓰면 좋다). 숫자 키패드가 필요하면 `inputMode="numeric"` 을 같이 준다.
- `Textarea` 에는 `allow` 가 없다. 여러 줄 입력은 문자 제한이 필요한 경우가 드물고,
  줄바꿈 처리까지 얽히기 때문이다. 필요해지면 같은 유틸로 확장한다.
...
- [ ] `Input allow` 는 문자 클래스(`/[a-z0-9]/`)로 주고, 형식 검증은 컨테이너·서버에서 다시 한다.
