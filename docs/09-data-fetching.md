...
> **`step` 을 바꿨는데 아이디/비밀번호 화면만 나온다면**
> 모르는 `step` 값은 1차 인증 화면으로 폴백된다. 값에 오타가 있거나(`"otpEnroll"` 등),
> 라이브러리 사본이 그 단계를 모르는 구버전일 때 생긴다. 이 경우 콘솔에
> `[LoginForm] 알 수 없는 step 값입니다: …` 경고가 한 번 찍히므로 먼저 콘솔을 확인한다.
> 경고가 안 찍히는데 화면도 안 바뀐다면 **라이브러리 사본 자체가 이 검사를 모르는 구버전**이다
> — 소비 시스템에 vendoring 한 사본을 갱신하고 빌드 산출물(`dist/`)까지 다시 만들었는지 본다.
> 타입(`LoginFormStep`)이 같은 사본에서 와야 `typecheck` 가 잘못된 값을 미리 잡아 준다.
